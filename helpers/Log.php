...
namespace Helpers;
// nella cartella logs verranno create le sottocartelle: auth, email, perf, php, signup, validation
/**
...
class Log
{
    ...
    private final function __construct() {
        ...
        // self::$baseURL = $_SERVER['DOCUMENT_ROOT']; // C:/xampp/htdocs
        // self::$baseURL = $_SERVER['DOCUMENT_ROOT'] . '/login-system/'; // C:/xampp/htdocs
        self::$baseURL = dirname($_SERVER['DOCUMENT_ROOT'], 1);
    ...
    /**
     * $directory:
     * performance: C:/xampp/htdocs/workspace/gestionale-custom/logs/perf/19/05
     * validation:  C:/xampp/htdocs/workspace/gestionale-custom/logs/validation/19/05
     * 
     * [b1] Crea la stringa che rappresenta la directory reale del file log
     * [b2] Se la directory già esiste ritorna null
     * altrimenti viene creata e ritorna true
     * [b3] Se la directory già esiste entra nel blocco di codice
     * @param string $kind
     * @param array $data
     * @return void
     */
    private function createLog(string $kind, array $data) {
        ...
        $directory = self::$baseURL . '/logs/' . $kind . '/' . self::$annoCorrente . '/' . self::$meseCorrente; // [b1]

        $return = $this->createPathIfNotExists($directory); // [b2]

        if (is_null($return)) { // [b3]
            $this->searchFilename($directory);
            $this->checkSizeOfFile($directory);
        }
    ...
    /**
     * [c1] Se la directory che contiene il file log già esiste, con il return esce fuori dal metodo
     ...
     * @return void
     */
    private function checkSizeOfFile($directory) {
